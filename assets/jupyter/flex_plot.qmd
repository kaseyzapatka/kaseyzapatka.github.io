...
These handle special coordinate transformations. `coord_flip = TRUE` rotates my plot 90 degrees, which is especially useful for bar charts with long category labels. Instead of tilting my head or rotating text at awkward angles, I just flip the coordinates and everything becomes readable.
...
One of the most useful additions I made to the function is built-in plot saving. Instead of creating a plot and then separately calling `ggsave()`, I can save the plot directly by specifying a file path relative to the working directory.
...
|-----------|-------------|---------|
| `data` | Your data frame | Required |
| `geom` | Geometry function (e.g., `geom_point`) | `geom_point` |
| `aes_main` | Aesthetic mappings as a list with `quote()` | `list()` |
| `facet_rows` | Facet by rows using `vars()` | `NULL` |
| `facet_cols` | Facet by columns using `vars()` | `NULL` |
| `scales` | List of scale functions | `list()` |
| `labs` | List of labels (title, x, y, caption, etc.) | All `NULL` |
| `labs$caption_wrap_width` | Character width for caption wrapping | `NULL` |
| `theme_fn` | Theme function or custom theme | Custom `theme_bw()` |
| `coord_flip` | Flip coordinates | `FALSE` |
| `coord_sf` | Use spatial coordinates for maps | `FALSE` |
| `save_path` | File path to save plot | `NULL` |
| `save_dpi` | Resolution for saved plots | `300` |
